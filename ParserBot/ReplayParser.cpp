...
#define ADD_AVAILABLE_TEC(MAT, RULE) mCurrentFeatures[GetPossibleActionIndex(MATERIAL::MAT)] = ((RULE) ? 1.f : 0.f);
...
#define ADD_AVAILABLE_UP(MAT, REQ) ADD_AVAILABLE_TEC(MAT, HAS_MAT(REQ) && !HAS_UPGRADE_MAXED(MAT) && !IS_RESEARCHING_UPGRADE(MAT))
    ...
    ADD_AVAILABLE_TEC(PLANETARYFORTRESS, HAS_MAT(ENGINEERINGBAY));
    ...
    ADD_AVAILABLE_TEC(STARPORT, HAS_MAT(STARPORT));
